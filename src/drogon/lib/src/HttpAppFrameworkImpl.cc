...
{
    LOG_INFO << "Initializing daemon mode";
#ifndef _WIN32
    if (getppid() != 1)
    {
        pid_t pid;
        ...
        if (pid > 0)
            throw std::runtime_error(
                "drogon: daemon parent should exit");  // parent
        if (pid < 0)
        {
            perror("fork");
            throw std::runtime_error("drogon: fork failed in daemon mode");
        }
        setsid();
    }
    ...
    throw std::runtime_error("drogon: daemon mode not supported on Windows");
#endif
...
static void TERMFunction(int sig)
{
    if (sig == SIGTERM)
    {
        LOG_TRACE << "SIGTERM signal received.";
        HttpAppFrameworkImpl::instance().getTermSignalHandler()();
    }
    else if (sig == SIGINT)
    {
        LOG_TRACE << "SIGINT signal received.";
    ...
    const std::string &outputPath)
{
    ...
    if (!libFileOutputPath_.empty())
    {
        if (drogon::utils::createPath(libFileOutputPath_) == -1)
        {
            ...
            throw std::runtime_error(
                "drogon: cannot create dynamic views output path");
        }
    }
    ...
    bool useSpdlog)
{
    ...
    {
        throw std::runtime_error("drogon: log path does not exist");
    }
    if (os_access(utils::toNativePath(logPath).c_str(), R_OK | W_OK) != 0)
    {
        throw std::runtime_error("drogon: unable to access log path");
    }
    logPath_ = logPath;
...
void HttpAppFrameworkImpl::run()
{
    ...
    if (relaunchOnError_)
    {
        ...
        while (true)
        {
            ...
            if (child_pid < 0)
            {
                ...
                throw std::runtime_error(
                    "drogon: fork failed in relaunch-on-error mode");
            }
            else if (child_pid == 0)
            {
                // child
    ...
    if (handleSigterm_)
    {
        ...
        if (sigaction(SIGINT, &sa, NULL) == -1)
        {
            ...
            throw std::runtime_error(
                "drogon: failed to install SIGINT handler");
        }
        if (sigaction(SIGTERM, &sa, NULL) == -1)
        {
            LOG_ERROR << "sigaction() failed, can't set SIGTERM handler";
            throw std::runtime_error(
                "drogon: failed to install SIGTERM handler");
        }
#endif
    }
    setupFileLogger();
...
HttpAppFramework &HttpAppFrameworkImpl::setupFileLogger()
{
        ...
        if (!trantor::Logger::getSpdLogger())
        {
            ...
            if (!logPath_.empty())
            {
                    ...
                    fsErr)
                {
                    ...
                    throw std::runtime_error(
                        "drogon: could not create log file path");
                }
                ...
                if (os_access(fsLogPath.native().c_str(), W_OK) != 0)
                {
                    ...
                    throw std::runtime_error(
                        "drogon: cannot create files in log folder");
                }
    ...
    if (!logPath_.empty() && !asyncFileLoggerPtr_)
    {
        ...
        if (os_access(utils::toNativePath(logPath_).c_str(), R_OK | W_OK) != 0)
        {
            ...
            throw std::runtime_error("drogon: log file path does not exist");
        }
        else
        {
